...
public class UserService {
    ...
    public void changeRights(User user) {
        userRepository.changeRights(user);
    }

    private String nameForAdmin(boolean admin) {
        return admin ? "disable" : "enable";
    }

    public User validateChangeRights(User user, String buttonLabel, String id) throws ValidationException {
        if (user == null) {
            throw new ValidationException("You should enter to change rights of users");
        }

        if (!user.isAdmin()) {
            throw new ValidationException("You should to be admin to change rights of users");
        }

        User changedUser;

        try {
            changedUser = find(Long.parseLong(id));
        } catch (NumberFormatException e) {
            throw new ValidationException("Id should contain only numbers");
        }

        if (!nameForAdmin(changedUser.isAdmin()).equals(buttonLabel)) {
