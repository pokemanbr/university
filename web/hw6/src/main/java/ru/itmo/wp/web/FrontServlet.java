...
public class FrontServlet extends HttpServlet {
    ...
    private void process(Route route,
        ...
        String[] methodsNames = {"before", route.getAction(), "after"};
        Map<String, Method> methods = new HashMap<>();
        for (String methodName: methodsNames) {
            for (Class<?> clazz = pageClass; !methods.containsKey(methodName) && clazz != null; clazz = clazz.getSuperclass()) {
                try {
                    methods.put(methodName, clazz.getDeclaredMethod(methodName, HttpServletRequest.class, Map.class));
        ...
        Method method = methods.get(route.getAction());
        try {
            for (String methodName: methodsNames) {
                methods.get(methodName).setAccessible(true);
                methods.get(methodName).invoke(page, request, view);
            }
