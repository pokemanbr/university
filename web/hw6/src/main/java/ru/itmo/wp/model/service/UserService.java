...
public class UserService {
    ...
    public void validateRegistration(User user, String password, String passwordConfirmation) throws ValidationException {
        ...
        if (user.getEmail().indexOf('@') == -1 || user.getEmail().indexOf('@') != user.getEmail().lastIndexOf('@')) {
            throw new ValidationException("Email should contain one @");
        }
        ...
        if (!password.equals(passwordConfirmation)) {
            throw new ValidationException("Passwords in the fields should be the same");
        }
    ...
    public boolean checkExistance(String login) {
        return userRepository.checkExistance(login);
    }
}
