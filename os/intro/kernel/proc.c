...
int dump(void) {
  ...
  struct trapframe *cur_trapframe = cur_proc->trapframe;
  uint64 *reg = &cur_trapframe->s2;
  for (int i = 2; i <= 11; i++) {
    printf("Register %d: %d\n", i, *reg);
    reg++;
  }
...
int dump2(int pid, int register_num, uint64 return_value) {
  ...
  for (p = proc; p < &proc[NPROC]; p++) {
    if (p->pid == pid) {
      if (p != myproc() && p->parent != myproc()) {
        return -1;
      }
      struct trapframe *cur_trapframe = p->trapframe;
      uint64 *reg = &cur_trapframe->s2;
      uint64 val = *(reg + (register_num - 2));
      if (copyout(myproc()->pagetable, return_value, (char *)&val,
                  sizeof(val)) < 0) {
        return -4;
      }
      return 0;
