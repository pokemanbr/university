...
int uvmcopypage(pagetable_t pagetable, uint64 address) {
  if (address >= MAXVA) return -1;
  ...
  if (getcntlink(pa) != 1) {
    ...
    kfree((void *)pa);
    uvmunmap(pagetable, address, 1, 0);
    if (mappages(pagetable, address, PGSIZE, (uint64)mem, flags) != 0) {
      return -1;
    }
  } else if (getcntlink(pa) == 1) {
    uvmunmap(pagetable, address, 1, 0);
    if (mappages(pagetable, address, PGSIZE, pa, flags) != 0) {
      return -1;
    }
  }
...
int copyout(pagetable_t pagetable, uint64 dstva, char *src, uint64 len) {
  ...
  while (len > 0) {
    ...
    if (pte == 0) return -1;
    if (*pte & PTE_RSW) {
      if (uvmcopypage(pagetable, va0) != 0) return -1;
    }
...
void walkpagetable(pagetable_t pagetable, uint64 lvl) {
  for (int i = 0; i < 512; i++) {
    pte_t pte = pagetable[i];
    if (pte & PTE_V) {
      printf(" ");
      ...
      if ((pte & (PTE_R | PTE_W | PTE_X)) == 0) {
        walkpagetable((pagetable_t)child, lvl + 1);
      }
    }
  }
}
