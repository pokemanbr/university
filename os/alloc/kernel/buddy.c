...
void bd_free(void *p) {
  ...
  for (k = size(p); k < MAXSIZE; k++) {
    ...
    void *left = (bi < buddy) ? p : addr(k, buddy);
    int pr = blk_index(k + 1, left);
    bit_invert(bd_sizes[k + 1].alloc, pr);
    if (bit_isset(bd_sizes[k + 1].alloc, pr)) {
      break;
    }
...
void bd_mark(void *start, void *stop) {
  ...
  for (int k = 0; k < nsizes; k++) {
    ...
    pi = blk_index(k + 1, start);
    for (; bi < bj; bi++) {
      if (k > 0) {
        // if a block is allocated at size k, mark it as split too.
        bit_set(bd_sizes[k].split, bi);
      }
      if (k + 1 < nsizes) bit_invert(bd_sizes[k + 1].alloc, pi);
      if (bi % 2) pi++;
    }
  }
}
...
int bd_initfree_pair(int k, int bi, int flag, int pr) {
  int buddy = (bi % 2 == 0) ? bi + 1 : bi - 1;
  int free = 0;
  if (bit_isset(bd_sizes[k + 1].alloc, pr)) {
...
int bd_initfree(void *bd_left, void *bd_right) {
  ...
  for (int k = 0; k < MAXSIZE; k++) {  // skip max size
    ...
    int pr = blk_index(k + 1, bd_left);
    free += bd_initfree_pair(k, left, 0, pr);
    if (right <= left) continue;
    pr = blk_index(k + 1, bd_right);
    free += bd_initfree_pair(k, right, 1, pr);
  }
  return free;
}
