...
inline unsigned char * encode_binary1(unsigned char * start, const uint8_t value)
{
...
inline unsigned char * encode_algo(unsigned char * start, const bool algo)
{
    return encode(start, static_cast<uint8_t>(algo ? 'Y' : 'N'));
}

inline constexpr size_t char_size = 1;
inline constexpr size_t algo_size = 1;
